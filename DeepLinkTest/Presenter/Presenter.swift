...
class Presenter {
    ...
    private var modulesNavigation:[String:NavigationProtocol] = [:]
    ...
    func removeModuleNavigation(rootID:String) {
        self.modulesNavigation.removeValue(forKey: rootID)
    }
    ...
    func initRootRouterNode(rootNode:RouteNode) {
        guard rootNode.routeNodeType == RouteNodeType.rootContainer else {
            return
        }
        configureContainerNode(containerNode: rootNode)
    }
    func routerModuleConfig(routerModule:RouterModule) {
        let modules = routerModule.getModuleRootNodes()
        for module in modules {
            
        }
    }
    func initRouteModules(routeModulesNodes:[RouterModule]) {
        for routerModule in routeModulesNodes {
            let rootNode = routerModule.routerModuleRootNode
            guard let navigationVC = viewPresenter.getView(routeNode: rootNode)
                as? NavigationProtocol else {
                    continue
            }
            modulesNavigation.updateValue(navigationVC,
                                          forKey: rootNode.routeNodeLink)
        }
    }
    func presentRouteModule(routerModule:RouterModule) {
        self.configureRouteModuleNavigation(routerModule: routerModule)
        self.configureContainerNode(containerNode: routerModule.routerModuleRootNode)
//        let rootVC = viewPresenter.getView(routeNode: routerModule.routerModuleRootNode)
//        self.rootViewController = rootVC
    }
    
    private func configureRouteModuleNavigation(routerModule:RouterModule) {
        for rootNode in routerModule.getModuleRootNodes() {
            guard let navigationVC = viewPresenter.getView(routeNode: rootNode) as? NavigationProtocol else {
                continue
            }
            modulesNavigation.updateValue(navigationVC, forKey: rootNode.routeNodeLink)
        }
    }
    ...
    private func configureContainerNode(containerNode:RouteNode) {
        ...
        for node in containerNode.containerForNodes {
            if (node.routeNodeType == RouteNodeType.root) {
                guard let rootVC = modulesNavigation[node.routeNodeLink] else {
                    continue
                }
    ...
    func presentRoutePath(routerPath:RouterPath, data:Any?) {
        guard let routerModuleNavigationVC = modulesNavigation[routerPath.rootNodeLink] else {
            return
        }
        let path = routerPath.path
        print("ROUTER PATH \(routerPath.path.count ) \(routerPath.pathType) \(routerPath.rootNodeLink)")
